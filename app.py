...
def check_distance(event):
    ...
    if distance(button_x, button_y, event.x, event.y) < 100:  # 100 is the distance threshold
        move_button()

def move_button():
    button.place(x=random.randint(0, window.winfo_width()-50), y=random.randint(0, window.winfo_height()-50))
